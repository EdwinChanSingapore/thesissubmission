...
d<- ggplot(df, aes(x=Type, y=Number, color=Chromosome)) +
  geom_bar(position="dodge", stat="identity",aes(fill=Chromosome), colour="light blue") +
  xlab("Chromosome") + ylab("Number Of Mutations in Chromosome") +
  theme(axis.text.x = element_text(size=14)) + ggtitle("Mutations in Simulated Genome") +
...
ggsave("C:\\Users\\User\\Documents\\thesis\\r_code\\MutationInSimulatedGenome.jpg")
